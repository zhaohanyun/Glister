...
def assess_img(image_file_path, plot=False, show_meta=False):
    ...
    dominant_pattern_idx = weight.squeeze().detach().numpy().argmax()+1
    dominant_pattern_score = weight.squeeze().detach().numpy().max()
    if show_meta:
        print("========== Image Analysis Results ============")
        print(f"=> Image Path: {image_file_path}")
        print(f"=> Object Detected: ")
        for obj_key, obj_score in obj_score_dict:
            print(f"\t {obj_key}({obj_score})")
        print(
            f"=> Dominant Patten is: pattern{dominant_pattern_idx} ({dominant_pattern_score})")
    ...
    return {
        ...
        "dominant_pattern_idx": dominant_pattern_idx,
        "dominant_pattern_score": dominant_pattern_score,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--show_meta', type=bool, required=False, default=False,
                        help='whether print out image analysis results.')

    args = parser.parse_args()
    img_results = assess_img(args.img_path, args.plot, args.show_meta)
